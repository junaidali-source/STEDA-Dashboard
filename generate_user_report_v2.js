...
async function generateReport() {
  try {
    ...
    // Query database through Supabase REST API
    // First, get all users with their activity counts
    console.log('Querying users...');
    const baseUrl = process.env.SUPABASE_URL;

    // Get users
    const usersUrl = `${baseUrl}/rest/v1/users?select=id,name,phone_number,phone_primary,region,subjects`;
    const dbUsers = await httpsRequest(usersUrl);
    console.log(`Found ${dbUsers.length} users in database`);

    // Get coaching sessions
    console.log('Fetching coaching sessions...');
    const coachingUrl = `${baseUrl}/rest/v1/coaching_sessions?select=user_id,session_date`;
    const coachingSessions = await httpsRequest(coachingUrl);

    // Get reading assessments
    console.log('Fetching reading assessments...');
    const raUrl = `${baseUrl}/rest/v1/reading_assessments?select=user_id`;
    const readingAssessments = await httpsRequest(raUrl);

    // Get lesson plans
    console.log('Fetching lesson plans...');
    const lprUrl = `${baseUrl}/rest/v1/lesson_plan_requests?select=user_id`;
    const lessonPlans = await httpsRequest(lprUrl);

    // Get image analysis
    console.log('Fetching image analysis...');
    const iaUrl = `${baseUrl}/rest/v1/image_analysis_requests?select=user_id`;
    const imageAnalysis = await httpsRequest(iaUrl);

    // Get video requests
    console.log('Fetching video requests...');
    const vrUrl = `${baseUrl}/rest/v1/video_requests?select=user_id`;
    const videoRequests = await httpsRequest(vrUrl);
